...
//	so they are listed here and shown by default.
		attribute "city", "string"			//SharpTool.ioSmartTiles
		attribute "feelsLike", "number"		//SharpTool.ioSmartTiles
		attribute "percentPrecip", "number"	//SharpTool.ioSmartTiles
		attribute "weather", "string"			//SharpTool.ioSmartTiles
		attribute "weatherIcon", "string"		//SharpTool.ioSmartTiles
		attribute "wind", "number"			//SharpTool.io

		//attribute "windDirection", "number"	//HubitatOpenWeather   NOW IN attrMap
		//attribute "windSpeed", "number"		//HubitatOpenWeather   NOW IN attrMap
		//attribute "forecastIcon", "string"	//SharpTool.io  NOW IN attrMap
		//attribute "weatherIcons", "string"		//HubitatopenWeather   NOW IN attrMap
...
// controlled with localSunrise
		...
		attribute "localSunrise", "string"	//SharpTool.ioSmartTiles (see below)
		attribute "localSunset", "string"		//SharpTool.ioSmartTiles  (see below)
...
//nearestStorm
	...
	preferences() {
		section("Query Inputs"){
			input "apiKey", "text", required: true, title: "DarkSky API Key"
			input "city", "text", required: true, defaultValue: "City or Location name forecast area", title: "City name"
			input "pollIntervalForecast", "enum", title: "DarkSky Poll Interval (daytime)", required: true, defaultValue: "3 Hours", options: ["Manual Poll Only", "2 Minutes", "5 Minutes", "10 Minutes", "15 Minutes", "30 Minutes", "1 Hour", "3 Hours"]
			input "pollIntervalForecastnight", "enum", title: "DarkSky Poll Interval (nighttime)", required: true, defaultValue: "3 Hours", options: ["Manual Poll Only", "2 Minutes", "5 Minutes", "10 Minutes", "15 Minutes", "30 Minutes", "1 Hour", "3 Hours"]
			...
			input "distanceFormat", "enum", required: true, defaultValue: "Miles (mph)", title: "Display Unit - Distance/Speed: Miles or Kilometers",options: ["Miles (mph)", "Kilometers (kph)"]
			...
			input "sourceImg", "bool", required: true, defaultValue: false, title: "Icons from: On = Source site - Off = Alternative"
			...
			input "settingEnable", "bool", title: "<b>Show All attribute options</b>", description: "$settingDescr", defaultValue: true
...
// <<<<<<<<<< Begin Setup Forecast Variables >>>>>>>>>>
	def fotime = new Date(ds.currently.time * 1000L)
	updateDataValue("fotime", fotime.toString())
	def futime = new Date(ds.currently.time * 1000L)
...
// <<<<<<<<<< Begin Process Standard Weather-Station Variables (Regardless of Forecast Selection)>>>>>>>>>>
	...
	BigDecimal ttt0 = ds.currently.windSpeed.toBigDecimal()
	if(ttt0 < 1.0) {
	...
	updateDataValue("wind", (isDistanceMetric ? (Math.round(ttt0 * 1.609344 * 10) / 10) : (Math.round(ttt0 * 10) / 10)).toString())
	...
	String w_cardinal
	String w_direction
	BigDecimal tt0 = ds.currently.windBearing.toBigDecimal()
	if(tt0 < 11.25) {
	...
	String s_cardinal
	String s_direction
//	if(nearestStormPublish) {  // someone may want to use this in myTile
		if(!ds.currently.nearestStormBearing){
			...
			BigDecimal t0 = ds.currently.nearestStormBearing.toBigDecimal()
			if(t0 < 11.25) {
...
//		}
	...
	String mPhase
//	if(moonPhasePublish){  // someone may want to use this in myTile
		BigDecimal t0 = ds.daily.data[0].moonPhase.toBigDecimal() * 100
		if (t0 < 6.25) {mPhase = "New Moon"}
		else if (t0 < 18.75) {mPhase = "Waxing Crescent"}
		else if (t0 < 31.25) {mPhase = "First Quarter"}
		else if (t0 < 43.75) {mPhase = "Waxing Gibbous"}
		else if (t0 < 56.25) {mPhase = "Full Moon"}
		else if (t0 < 68.75) {mPhase = "Waning Gibbous"}
		else if (t0 < 81.25) {mPhase = "Last Quarter"}
		else if (t0 < 93.75) {mPhase = "Waxing Gibbous"}
		else if (t0 >= 93.75) {mPhase = "New Moon"}
...
// >>>>>>>>>> End Process Standard Weather-Station Variables (Regardless of Forecast Selection)<<<<<<<<<<
	...
	if (!ds.currently.cloudCover) {
		...
		cloudCover = (ds.currently.cloudCover.toBigDecimal() <= 0.01) ? 1 : Math.round(ds.currently.cloudCover.toBigDecimal() * 100)
	}
	...
	String c_code
	boolean cur_day = getDataValue("is_day")=="1" ? true : false
	switch(ds.currently.icon) {
		case "clear-day": c_code = "sunny"; break;
		case "clear-night": c_code = "nt_clear"; break;
		case "rain": c_code = (cur_day ? "rain" : "nt_rain"); break;
		case "wind": c_code = (cur_day ? "breezy" : "nt_breezy"); break;
		case "snow": c_code = (cur_day ? "snow" : "nt_snow"); break;
		case "sleet": c_code = (cur_day ? "sleet" : "nt_sleet"); break;
		case "fog": c_code = (cur_day ? "fog" : "nt_fog"); break;
		case "cloudy": c_code = (cur_day ? "cloudy" : "nt_cloudy"); break;
	...
	switch(ds.daily.data[0].icon){
		...
		case "rain": f_code = (cur_day ? "rain" : "nt_rain"); break;
		case "wind": f_code = (cur_day ? "breezy" : "nt_breezy"); break;
		case "snow": f_code = (cur_day ? "snow" : "nt_snow"); break;
		case "sleet": f_code = (cur_day ? "sleet" : "nt_sleet"); break;
		case "fog": f_code = (cur_day ? "fog" : "nt_fog"); break;
		case "cloudy": f_code = (cur_day ? "cloudy" : "nt_cloudy"); break;
	...
	//if(precipExtendedPublish){  // someone may want to use this in myTile
...
// >>>>>>>>>> End Setup Forecast Variables <<<<<<<<<<
	...
	if(sourceImg==false){ // 'Alternative' Icons selected
		theImgUrl = (getDataValue("iconType")== 'true' ? getImgUrl(getDataValue("condition_code")) : getImgUrl(getDataValue("forecast_code")))
		sendEventPublish(name: "condition_icon", value: '<img src=' + theImgUrl + '>')
		sendEventPublish(name: "condition_iconWithText", value: "<img src=" + theImgUrl + "><br>" + (getDataValue("iconType")== 'true' ? getDataValue("condition_text") : getDataValue("forecast_text")))
		sendEventPublish(name: "condition_icon_url", value: theImgUrl)
		updateDataValue("condition_icon_url", theImgUrl)
		sendEventPublish(name: "condition_icon_only", value: theImgUrl.split("/")[-1].replaceFirst("\\?raw=true",""))
...
/*Selected optional Data Elements */
	sendEventPublish(name: "DSattribution", value: '<a href=\"https://darksky.net/poweredby/\"><img src=' + getDataValue("iconLocation") + (dsIconbackgrounddark ? 'poweredby-oneline.png' : 'poweredby-oneline-darkbackground.png') + ' style=\"height:1.5em\";></a>')
	...
	sendEventPublish(name: "forecastIcon", value: getDataValue("forecast_code"))
	...
	sendEventPublish(name: "weatherIcons", value: getowmImgName(getDataValue("condition_code")))
	sendEventPublish(name: "wind_degree", value: getDataValue("wind_degree").toInteger(), unit: "DEGREE")
	sendEventPublish(name: "windDirection", value: getDataValue("wind_degree").toInteger(), unit: "DEGREE")
	...
	sendEventPublish(name: "windSpeed", value: getDataValue("wind").toBigDecimal(), unit: (isDistanceMetric ? 'KPH' : 'MPH'))
...
//<<<<<<<<<< Begin Built Weather Summary text >>>>>>>>>>
	String futime = getDataValue("futime")
	String Summary_last_poll_time = new Date().parse("EEE MMM dd HH:mm:ss z yyyy", "${futime}").format(timeFormat, TimeZone.getDefault())
	String Summary_last_poll_date = new Date().parse("EEE MMM dd HH:mm:ss z yyyy", "${futime}").format(dateFormat, TimeZone.getDefault())
	mtprecip = getDataValue("percentPrecip") + '%'
	if(weatherSummaryPublish){ // don't bother setting these values if it's not enabled
		String Summary_forecastTemp = " with a high of " + getTempStr("forecastHigh") + " and a low of " + getTempStr("forecastLow")
...
String getTempStr(String dataVal) {
	String t0 =  String.format("%3.1f", getDataValue(dataVal).toBigDecimal()) + (isFahrenheit ? '°F' : '°C')
	return t0
}
...
public void setMeasurementMetrics(distFormat, pressFormat, precipFormat, temptFormat){
	...
	isRainMetric = (precipFormat == "Millimeters") ? true : false
	isFahrenheit = (temptFormat == "Fahrenheit (°F)") ? true : false
	return
}

def estimateLux(String condition_code, String cloud) {
	long lux = 0l
	boolean aFCC = true
	def l
	String bwn
	...
	if(aFCC){
		if(cloud !="" && cloud != null){
			LUitem = LUTable.find{ (String)it.wucode == condition_code && (int)it.day == 1 }
...
void SummaryMessage(boolean SType, String Slast_poll_date, String Slast_poll_time, String SforecastTemp, String Sprecip, String Svis){
	...
	if(SType == true){
		...
		wSum+= "Humidity is " + getDataValue("humidity") + "% and the temperature is " + getTempStr("temperature")
		wSum+= "The temperature feels like it is " + getTempStr("feelsLike")
		...
		wSum = getDataValue("condition_text") + " "
		wSum+= ((!SforecastTemp || SforecastTemp=="") ? ". " : "${SforecastTemp}")
		wSum+= " Humidity: " + getDataValue("humidity") + "%. Temperature: " + getTempStr("temperature")
...
String getImgUrl(String wCode){
	LOGINFO("getImgUrl Input: wCode: " + wCode + "state.is_day: " + getDataValue("is_day") + " iconLocation: " + getDataValue("iconLocation"))
	LUitem = LUTable.find{ (String)it.wucode == wCode } //&& it.day.toString() == getDataValue("is_day") }
	String result = (getDataValue("iconLocation") + (LUitem ? LUitem.img : 'na.png') + (((getDataValue("iconLocation").toLowerCase().contains('://github.com/')) && (getDataValue("iconLocation").toLowerCase().contains('/blob/master/'))) ? "?raw=true" : ""))
	LOGINFO("getImgUrl Result: image url: " + result)
	return result
}

String getowmImgName(String wCode){
	LOGINFO("getowmImgName Input: wCode: " + wCode)
	LUitem = LUTable.find{ (String)it.wucode == wCode } //&& it.day.toString() == getDataValue("is_day") }
	LOGINFO("getowmImgName Result: image name: " + (LUitem ? LUitem.owm : '') )
...
@Field static attributesMap = [
	...
	"forecastIcon":			[title: "Forecast Icon", descr: "Display 'forecastIcon'? (same attribute is 'forecast_code')", typeof: "string", default: "false"],
	...
	"weatherIcons":			[title: "Condition Icon", descr: "Display 'weatherIcons'?", typeof: "string", default: "false"],
	...
	"windDirection":		[title: "Wind Direction (Degree)", descr: "Display the Wind Direction (number)? (same attribute is 'wind_degree')", typeof: "number", default: "false"],
	...
	"windSpeed":			[title: "Wind speed", descr: "Display the 'windSpeed'? (already in attribute 'wind')", typeof: "string", default: "false"],
	"verUpdate":			[title: "Software update", descr: "Display the 'verUpdate'?", typeof: "string", default: "false"],
	"verStatus": 			[title: "Software status", descr: "Display the 'verStatus'?", typeof: "string", default: "false"],
	"DSattribution":		[title: "Attribution", descr: "Display the 'DSattribution'?", typeof: "string", default: "false"],
]
...
def updateCheckHandler(resp, data) {
	...
	String InternalName = "DarkSky.net Weather Driver"
	...
	if (resp.getStatus() == 200 || resp.getStatus() == 207) {
		...
		def newVer = padVer(respUD.driver.("${InternalName}").ver)
		def currentVer = padVer(version())
		String UpdateInfo = (respUD.driver.("${InternalName}").updated)
		state.UpdateInfo = UpdateInfo
			// log.debug "updateCheck: ${respUD.driver.(InternalName).ver}, $UpdateInfo, ${respUD.author}"
		...
		switch(newVer) {
			...
			case { it > currentVer}:
				...
				if (descTextEnable) log.warn "** There is a newer version of this Driver available(Version: ${newVer}) **"
				if (descTextEnable) log.warn "** $UpdateInfo **"
				break
			case { it < currentVer}:
				state.Status = "<b>You are using a Test version of this Driver (Expecting: ${newVer})</b>"
				if (descTextEnable) log.warn "You are using a Test version of this Driver (Expecting: ${newVer})"
				break
			default:
				state.Status = "Current Version: ${newVer}"
				if (descTextEnable) log.info "You are using the current version of this driver"
				break
		}

		sendEventPublish(name: "verUpdate", value: UpdateInfo)
		sendEventPublish(name: "verStatus", value: state.Status)
	}else{
